...
def main():
    ...
    # Tab 1: Upload Data
    with tab1:
        st.header("📁 Upload or Paste Your CSV Data")
        ...
        # --- Option A: File uploader (may fail behind reverse proxy) ---
        uploaded_file = st.file_uploader(
            "Choose a CSV file (may not work behind reverse proxy)",
            type=['csv'],
        ...
        csv_text = st.text_area(
            "Or paste CSV content here (for small files)",
            height=200,
            placeholder="Paste CSV data including header row..."
        )
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                st.success("✅ CSV loaded from pasted text!")
            except Exception as e:
                st.error(f"Error parsing pasted CSV: {str(e)}")

        # If we got a dataframe, save to session and show preview
        if df is not None:
        ...
        with tab5:
            ...
            <div class="feature-card">
                ...
                <ul>
                    ...
                    <li><strong>Upload or Paste</strong> - Use files or paste with ---SNAPSHOT--- delimiter</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
            # Snapshot upload section
            st.subheader("📁 Upload or Paste Dataset Snapshots")
            # Upload/Paste section
            # --- Option A: File uploader for multiple snapshots ---
            uploaded_snapshots = st.file_uploader(
                "Upload Multiple Dataset Snapshots (CSV files - may not work behind reverse proxy)",
                type=['csv'],
            ...
            # --- Option B: Paste multiple CSV snapshots ---
            csv_snapshots_text = st.text_area(
                "Or paste multiple CSV snapshots here (separate each with ---SNAPSHOT---)",
                height=300,
                placeholder="Paste first CSV snapshot...\n---SNAPSHOT---\nPaste second CSV snapshot...\n---SNAPSHOT---\nPaste third CSV snapshot...",
                help="Paste multiple CSV snapshots separated by ---SNAPSHOT--- delimiter",
                key="snapshot_paste"
            )
            
            # Process uploaded snapshots from file uploader
            if uploaded_snapshots and len(uploaded_snapshots) >= 2:
                if st.button("🔄 Process Uploaded Snapshots", help="Analyze uploaded snapshots for historical comparison", key="process_uploaded"):
                    snapshots_data = []
            ...
            # Process pasted CSV snapshots
            elif csv_snapshots_text.strip():
                # Split by delimiter
                snapshot_chunks = csv_snapshots_text.split('---SNAPSHOT---')
                snapshot_chunks = [chunk.strip() for chunk in snapshot_chunks if chunk.strip()]
                
                if len(snapshot_chunks) >= 2:
                    if st.button("🔄 Process Pasted Snapshots", help="Analyze pasted snapshots for historical comparison", key="process_pasted"):
                        snapshots_data = []
                        
                        with st.spinner("Processing pasted snapshots..."):
                            for i, csv_chunk in enumerate(snapshot_chunks):
                                try:
                                    # Parse CSV from text
                                    from io import StringIO
                                    ...
                                    snapshot_name = f"Pasted_Snapshot_{i+1}"
                                    ...
                                    snapshot_data = {
                                        ...
                                        'name': snapshot_name,
                                        'data': df,
                                        'timestamp': datetime.now() - pd.Timedelta(days=len(snapshot_chunks)-i-1),  # Simulate time progression
                                        'row_count': len(df),
                                        'column_count': len(df.columns),
                                        'completeness': quality_metrics['completeness'],
                                        'uniqueness': quality_metrics['uniqueness'],
                                        'overall_score': round((quality_metrics['completeness'] + quality_metrics['uniqueness']) / 2, 1),
                                        'type_distribution': quality_metrics['type_distribution']
                                    }
                                    ...
                                    st.error(f"Error processing snapshot {i+1}: {str(e)}")
                        
                        # Store snapshots in session state
                        if snapshots_data:
                            st.session_state.dataset_snapshots = snapshots_data
                            st.success(f"✅ Successfully processed {len(snapshots_data)} pasted snapshots!")
                else:
                    st.info(f"📝 Found {len(snapshot_chunks)} snapshot(s). Please paste at least 2 snapshots separated by ---SNAPSHOT---")
